...
(async () => {
  ...
  let settings = { delaySec: 5, imageCount: 1 };
  ...
  function saveSettings() {
    settings = {
      delaySec: parseFloat($("delaySec").value) || 5,
      imageCount: parseInt($("imageCount").value) || 1,
    };
    chrome.storage.local.set({ [SETTINGS_KEY]: settings });
  }
  ["delaySec","imageCount"].forEach(id => {
    ...
    qItems.forEach((item, idx) => {
      ...
      div.innerHTML = `
        ...
        <div class="q-body">
          ...
          <div class="q-status">${statusLabel}${item.status === "error" && item.error ? " — " + escHtml(item.error) : ""}</div>
        </div>
  ...
  async function checkConn(silent=false) {
    if (!silent) setStatus("checking","Checking Flow…");
    const ping = await sendMsg({ type:"PING" });
    if (!ping?.pong) { setStatus("disconnected","Extension error — reload"); return { status:"disconnected" }; }
    const res = await sendMsg({ type:"INSPECT_FLOW" });
    const s = res?.state || { status:"disconnected" };
    const page = s.page || {};
    // While the queue is active, keep the live automation state visible instead
    // of allowing the 3-second connection poll to overwrite it with "Connected".
    if (isRunning) return s;
    if (page.flowDetected && page.projectDetected && page.editorDetected) {
      setStatus("connected","Connected");
    } else if (page.flowDetected && page.projectDetected) {
      setStatus("connected","Flow connected — finding composer…");
    } else if (s.contentScript && s.flowTabId) {
      setStatus("connected","Flow connected — open a project");
    } else {
      setStatus("disconnected","Not connected — open Flow");
    }
    return s;
  }
  await checkConn();
  ...
  $("btn-start").addEventListener("click", async () => {
    ...
    if (!s?.flowTabId) { alert("Open flow.google.com and open a project."); return; }
    ...
    setRunning(true);
    setStatus("automation", "Automation running");
  ...
  chrome.runtime.onMessage.addListener(msg => {
    ...
    if (msg.type === "BATCH_PROGRESS") {
      ...
      setStatus("generating", `📸 Generating photo ${msg.current}/${msg.total}`);
    ...
    if (msg.type === "COUNTDOWN_TICK") {
      setStatus("automation", `⚡ Next photo in ${Math.ceil(msg.msLeft/1000)}s`);
    ...
    if (msg.type === "ITEM_ERROR") {
      const item = qItems.find(x=>x.id===msg.id);
      if (item) { item.status = "error"; item.error = msg.error; }
      renderQueue();
    }
    if (msg.type === "BATCH_ERROR") {
      ...
      setStatus("connected", "Connected");
      hideCd();
      $("prog-status").textContent = `⚠ Stopped: ${msg.error || "unknown error"}`;
      $("prog-prompt").textContent = msg.prompt || "";
    }
